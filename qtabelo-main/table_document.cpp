...
void TableDocument::slotAddTab(const int count)
{
    if (m_tabs->count() == 0) {
        for (int i = 0; i < count; ++i) {
            auto* table = new QTableWidget(15,5);
...
void TableDocument::removeSelectedRows(QTableWidget *table)
{
...
void TableDocument::removeSelectedColumns(QTableWidget *table)
{
    QList<QTableWidgetSelectionRange> ranges = table->selectedRanges();

    QSet<int> columnsToRemove;
    for (const QTableWidgetSelectionRange &range : ranges) {
        for (int col = range.leftColumn(); col <= range.rightColumn(); ++col) {
            columnsToRemove.insert(col);
        }
    }

    QList<int> sortedColumns = columnsToRemove.values();
...
void TableDocument::updateColumnHeaders(QTableWidget *table)
{
    int columnCount = table->columnCount();
    for (int col = 0; col < columnCount; ++col) {
        QTableWidgetItem *headerItem = table->horizontalHeaderItem(col);
        if (headerItem) {
            headerItem->setText(QString("Column %1").arg(col + 1));
        } else {
            table->setHorizontalHeaderItem(col, new QTableWidgetItem(QString("Column %1").arg(col + 1)));
        }
    }
}
void TableDocument::showTableContextMenu(QTableWidget *table, const QPoint &pos)
{
    ...
    QAction *actionRemoveSelectedRows = menu.addAction(tr("Remove Selected Rows"));
    QAction *actionRemoveSelectedColumns = menu.addAction(tr("Remove Selected Columns"));

    QList<QTableWidgetSelectionRange> selectedRanges = table->selectedRanges();
    bool hasSelection = !selectedRanges.isEmpty();
    actionRemoveSelectedRows->setEnabled(hasSelection);
    actionRemoveSelectedColumns->setEnabled(hasSelection);
    ...
    else if (selectedAction == actionRemoveSelectedRows) {
        removeSelectedRows(table);
        emit contentChanged(true);
    }
    else if (selectedAction == actionRemoveSelectedColumns) {
